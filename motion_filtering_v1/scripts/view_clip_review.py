...
from common.ffmpeg_utils import parse_crop_box  # noqa: E402
from common.video_files import find_movie_video  # noqa: E402
from common.video_time import clip_local_range  # noqa: E402
...
def load_records(meta_path: Path, only_final: bool, only_tagged: bool) -> list[dict]:
    ...
    return [r for r in rows if r.get("caption")]
...
def export_clip(
    ...
    *,
    ...
    start, end = clip_local_range(rec, None)
    duration = max(0.1, end - start)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    vf_parts: list[str] = []
    crop = parse_crop_box(rec.get("crop_box", ""))
    if crop:
        w, h, x, y = crop
    ...
    try:
        ...
        print(f"ffmpeg failed {rec['clip_id']}: {err}")
        return False
...
def main() -> None:
    ...
    if not records:
        raise SystemExit("No matching records with captions")
    ...
    for rec in picked:
        ...
        if not args.skip_export:
            if clip_path.exists() and not args.force_export:
                ...
                ok = export_clip(source, rec, clip_path)
                if ok:
                    exported += 1
                else:
                    failed += 1
        ...
        rel = f"clips/{rec['clip_id']}.mp4"
        items.append((rec, rel))
